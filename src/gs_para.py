'''
    1. Physics-encoded Recurrent Conv Neural Network for space-varying coefficients discovery.
...
class RCNNCell(nn.Module):
    '''
        ...
        Completely based on physics (only PDE coefficients are unknown) and RK4 scheme
    '''
    ...
    def __init__(self, dt=0.5, dx=0.01):
        ...
        # self.DB = self.DA / 4
        # self.f = 1 / 25
        # self.k = 0.063 * np.exp(- (x_star ** 2 + y_star ** 2) / 2)
        ...
        # define a space-varying k
        self.ck_A = torch.nn.Parameter(torch.tensor(0.1*np.random.rand(), dtype=torch.float64), requires_grad=True)
        
        # define X
        # set gaussian distribution on DA
        x = np.linspace(-0.5, 0.5, 101)[:-1]
        y = np.linspace(-0.5, 0.5, 101)[:-1]
        x_star, y_star = np.meshgrid(x, y)
        self.star_mesh = torch.from_numpy(np.exp(- (x_star ** 2 + y_star ** 2) / 2)).cuda()
        self.star_mesh.requires_grad = False
        
        # self.k = self.ck_A * star_mesh
        # self.k = self.k.cuda()
        # self.k = torch.nn.Parameter(torch.tensor(0.1*np.random.rand()*star_mesh, dtype=torch.float64), requires_grad=True)
    ...
    def f_rhs(self, u, v):
        ...
        f_v = self.mu_up*torch.sigmoid(self.CB)*self.W_laplace(v) + self.NB* u*v**2 - (self.star_mesh*self.ck_A+self.f)*v

        return f_u, f_v
    
    def forward(self, h, method='rk2'):
    ...
    def show_coef(self):
        ...
        table = PrettyTable()
        table.field_names = ['\\', r"$\mu_u$", r"$\mu_v$", r"$C_1$", r"$C_2$", r"$C_F$", r"$Ck_A$", ]
        mu_u, mu_v, C_1, C_2, C_F, Ck_A = self.mu_up*torch.sigmoid(self.CA).item(), \
                                         self.mu_up * torch.sigmoid(self.CB).item(), \
                                         self.NA.item(), self.NB.item(), self.f.item(), self.ck_A.item()
        table.add_row(["True",       2E-5, 5E-6, -1.00, 1.00, 0.04, 0.063])
        table.add_row(["Identified", mu_u, mu_v, C_1, C_2, C_F, Ck_A])
        print(table)
        return mu_u, mu_v, C_1, C_2, C_F, Ck_A
...
class RCNN(nn.Module):
    ...
    def forward(self, init_state_low, method='rk2'):
